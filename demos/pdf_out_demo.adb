...
procedure PDF_Out_Demo is
  ...
  procedure Big_Demo is
    ...
    procedure Boarding_Pass_Demo is
    ...
    end Lines_and_Shapes_Demo;

    procedure Bezier_Curves_Demo is
      procedure Bezier_Curves_Figure (o : Point) is
        f : constant := 0.3;
      begin
    ...
    begin
      ...
      ------------------------------
      Front_Page;
      pdf.Finish_Page;  --  Needed for having a correct footer before changing orientation.
      ...
      pdf.Page_Setup (A4_landscape);
      pdf.New_Page;
      pdf.Put ("Direct PDF vector graphics code inclusion - the Ada Mascot");
      for cx in 1 .. 8 loop
        for cy in 1 .. 6 loop
          Insert_Mascot
            (pdf,
      ...
      --  Finishing
      --
      mem_page_nb := pdf.Page;  --  Remember the total number of pages.
      pdf.Close;
    end Big_Demo_Contents;

  begin
    --  Dry run: here page_nb (for the footer) and the page index (for the
    --  inter-document hyperlinks) are set properly (TeX-style processing).
    --  Ideally, the "dry run" file is set to a Null file like
    --  /dev/null (UNIX) or nul (Windows). Absent a portable solution, we
    --  write the same document twice.
    --
    Big_Demo_Contents ("Big.pdf");

    --  Real file with correct total page number and
